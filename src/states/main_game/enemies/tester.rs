use cgmath::{ Angle, EuclideanSpace, InnerSpace, Rad, Point2, vec2 };
...
impl Tester {
    ...
    pub fn update(&mut self, player : &Player) {
        ...
        let ang = vec2(0.0f32, 1.0f32).angle(player_pos.to_vec());
        let (s, c) = (ang + Rad(std::f32::consts::FRAC_PI_2)).sin_cos();

        if self.hp > 0 {
            self.ang = ang; 
            self.pos += vec2(c, s) * TESTER_STEP_LENGTH;
        } else {
