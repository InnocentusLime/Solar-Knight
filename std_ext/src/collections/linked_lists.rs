...
/// `LinkedLists` is a structure which keeps `n` linked
/// lists in a cache friendly way. Basically, this 
/// structure contains `n` linked lists and all their elements
/// are placed in the memory continiously.
///
...
impl<T> LinkedLists<T> {
    ...
    pub fn push(&mut self, x : T, slot_id : usize) -> usize {
        let new_cell = Node { val : x, next : None, prev : None, slot_id };
        let new_id = 
